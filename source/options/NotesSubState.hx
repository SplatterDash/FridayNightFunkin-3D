...
class NotesSubState extends MusicBeatSubstate
{
	...
	public function new() {
		...
		colorPalette = new FlxSprite(820, 580).loadGraphic(Paths.image('noteColorMenu/palette', null, false));
		...
		spawnNotes();
		updateNotes(true);
		FlxG.sound.play(Paths.sound('scrollMenu'), 0.6 * ClientPrefs.data.soundVolume);
	...
	override function update(elapsed:Float) {
		...
		else if(FlxG.mouse.justPressed || FlxG.mouse.justMoved) controls.controllerMode = false;
		//
		...
		else if (pointerOverlaps(pasteButton))
		{
			...
			if(generalPressed)
			{
				...
				if(newColor != null && formattedText.length == 6)
				{
					...
					FlxG.sound.play(Paths.sound('cancelMenu'), 0.6 * ClientPrefs.data.soundVolume);
			}
			hexTypeNum = -1;
		}
