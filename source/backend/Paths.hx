...
class Paths
{
	...
	[
		'assets/music/3dmainmenu.$SOUND_EXT',
	...
	public static function getPath(file:String, ?type:AssetType = TEXT, ?library:Null<String> = null, ?modsAllowed:Bool = false):String
	{
		...
		if (currentLevel != null)
		{
			...
			if(currentLevel != 'shared') {
				levelPath = getLibraryPathForce(file, null, currentLevel);
	...
	{
		if(level == null && library != null) level = library;
		var returnPath = (library != null ? '$library:' : '') + 'assets/' + (level != null ? '$level/' : '') + file;
		return returnPath;
	}
	...
	{
		#if html5
		return 'songs:assets/songs/${formatToSongPath(song)}/Vocals.$SOUND_EXT';
		#else
		var songKey:String = '${formatToSongPath(song)}/Vocals';
	...
	static public function image(key:String, ?library:String, ?allowGPU:Bool = true):FlxGraphic
		{
			...
			{
				localTrackedAssets.push(file);
				if (allowGPU && ClientPrefs.data.cacheOnGPU && FlxG.stage.context3D != null)
				{
	...
	static public function getAtlas(key:String, ?callCard:String, ?library:String = null):FlxAtlasFrames
	{
