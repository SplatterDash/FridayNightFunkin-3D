...
class ResetScoreSubState extends MusicBeatSubstate
{
	...
	// Week -1 = Freeplay
	public function new(song:String, character:String, storyMode:Bool = false)
	{
		this.song = song;
		...
		text.scaleX = tooLong;
		text.screenCenter(X);
		if(!storyMode) text.x += 60 * tooLong;
		alphabetArray.push(text);
		text.alpha = 0;
		add(text);
		if(!storyMode) {
	...
	override function update(elapsed:Float)
	{
		...
		if(!storyMode) icon.alpha += elapsed * 2.5;
	...
	function updateOptions() {
		...
		if(!storyMode) icon.animation.curAnim.curFrame = confirmInt;
	}
}