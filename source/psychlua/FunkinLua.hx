...
class FunkinLua {
	...
	public function new(scriptName:String) {
			...
			if(game.modchartSprites.exists(tag)) {
				...
				else
				{
					if(!game.isDead)
						...
						GameOverSubstate.instance.insert(GameOverSubstate.instance.members.indexOf(GameOverSubstate.instance.deathSprite), shit);
				}
			}
		});
		...
		Lua_helper.add_callback(lua, "playMusic", function(sound:String, volume:Float = 1, loop:Bool = false) {
			FlxG.sound.playMusic(Paths.music(sound), volume * ClientPrefs.data.musicVolume, loop);
		});
		Lua_helper.add_callback(lua, "playSound", function(sound:String, volume:Float = 1, ?tag:String = null) {
			if(tag != null && tag.length > 0) {
				...
				game.modchartSounds.set(tag, FlxG.sound.play(Paths.sound(sound), volume * ClientPrefs.data.soundVolume, false, function() {
					game.modchartSounds.remove(tag);
					game.callOnLuas('onSoundFinished', [tag]);
				}));
				return;
			}
			FlxG.sound.play(Paths.sound(sound), volume * ClientPrefs.data.soundVolume);
		});
	...
	public function initLuaShader(name:String, ?glslVersion:Int = 120)
	{
		...
		luaTrace('You took out the runtime shader support because you took out mod support. Idior.', false, false, FlxColor.RED);
		return false;
	}
}