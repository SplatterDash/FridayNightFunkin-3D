...
class ChartingState extends MusicBeatState
{
	...
	[
		...
		['BG Lights', 'Philly Li- I mean, 17 bucks!'],
		['Scanline Moment', 'Just put in this event and the game will do the rest!'],
		['Play Video BG', 'Plays a video in the background!'],
		['Change BG Color', 'Permanently change the BG\'s color.\nUnless, of course, you want to change it again.'],
		['Pulse BG Color', 'Temporarily change the BG\'s color.'],
		['Camera Snap To', 'Makes camera snap to a position.\nThe position can either be a character or a point.'],
		['Outline Shaders', 'Turns the outline shader for the characters on or off.'],
	...
	**/
	...
	function addDataUI()
	{
		...
		gameOverLoopInputText = new FlxUIInputText(10, gameOverSoundInputText.y + 35, 150, _song.delayAnimation != null ? Std.string(_song.delayAnimation) : "0", 8);
	...
	function generateSong() {
		FlxG.sound.playMusic(Paths.inst(currentSongName), 0.6 * ClientPrefs.data.musicVolume/*, false*/);
	...
	override function getEvent(id:String, sender:Dynamic, data:Dynamic, ?params:Array<Dynamic>)
	{
		...
		else if (id == FlxUINumericStepper.CHANGE_EVENT && (sender is FlxUINumericStepper))
		{
			...
			if(sender == noteSplashesInputText) {
				...
				_song.delayAnimation = Std.parseFloat(gameOverLoopInputText.text);
			}
	...
	override function update(elapsed:Float)
	{
				...
				if(note.strumTime > lastConductorPos && FlxG.sound.music.playing && note.noteData > -1) {
					...
					if(!playedSound[data]) {
						if(note.hitsoundChartEditor && ((playSoundBf.checked && note.mustPress) || (playSoundDad.checked && !note.mustPress)))
						{
							...
							FlxG.sound.play(Paths.sound(soundToPlay), 1 * ClientPrefs.data.soundVolume).pan = note.noteData < 4? -0.3 : 0.3; //would be coolio
		...
		if(metronome.checked && lastConductorPos != Conductor.songPosition) {
			...
			if(metroStep != lastMetroStep) {
				FlxG.sound.play(Paths.sound('Metronome_Tick'), 1 * ClientPrefs.data.soundVolume);
				//trace('Ticked');
			}
		}
